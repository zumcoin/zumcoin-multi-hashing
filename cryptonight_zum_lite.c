...
// Portions Copyright (c) 2019, ZumCoin Development Team
...
static void do_zum_lite_blake_hash(const void* input, size_t len, char* output) {
    blake256_hash((uint8_t*)output, input, len);
}

void do_zum_lite_groestl_hash(const void* input, size_t len, char* output) {
    groestl(input, len * 8, (uint8_t*)output);
}

static void do_zum_lite_jh_hash(const void* input, size_t len, char* output) {
    int r = jh_hash(HASH_SIZE * 8, input, 8 * len, (uint8_t*)output);
    assert(SUCCESS == r);
}

static void do_zum_lite_skein_hash(const void* input, size_t len, char* output) {
    int r = c_skein_hash(8 * HASH_SIZE, input, 8 * len, (uint8_t*)output);
    assert(SKEIN_SUCCESS == r);
}

static void (* const extra_hashes[4])(const void *, size_t, char *) = {
    do_zum_lite_blake_hash, do_zum_lite_groestl_hash, do_zum_lite_jh_hash, do_zum_lite_skein_hash
};
...
#if defined(_MSC_VER)
    struct cryptonightzumlite_ctx *ctx = _malloca(sizeof(struct cryptonightzumlite_ctx));
#else
    struct cryptonightzumlite_ctx *ctx = alloca(sizeof(struct cryptonightzumlite_ctx));
#endif
