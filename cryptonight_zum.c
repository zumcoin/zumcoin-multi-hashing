...
static void do_zum_blake_hash(const void* input, size_t len, char* output) {
    blake256_hash((uint8_t*)output, input, len);
}

void do_zum_groestl_hash(const void* input, size_t len, char* output) {
    groestl(input, len * 8, (uint8_t*)output);
}

static void do_zum_jh_hash(const void* input, size_t len, char* output) {
    int r = jh_hash(HASH_SIZE * 8, input, 8 * len, (uint8_t*)output);
    assert(SUCCESS == r);
}

static void do_zum_skein_hash(const void* input, size_t len, char* output) {
    int r = c_skein_hash(8 * HASH_SIZE, input, 8 * len, (uint8_t*)output);
    assert(SKEIN_SUCCESS == r);
}

static void (* const extra_hashes[4])(const void *, size_t, char *) = {
    do_zum_blake_hash, do_zum_groestl_hash, do_zum_jh_hash, do_zum_skein_hash
};
...
#if defined(_MSC_VER)
    struct cryptonightzum_ctx *ctx = _malloca(sizeof(struct cryptonightzum_ctx));
#else
    struct cryptonightzum_ctx *ctx = alloca(sizeof(struct cryptonightzum_ctx));
#endif
