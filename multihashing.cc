...
extern "C" {
    ...
    #include "cryptonight_zum.h"
    #include "cryptonight_zum_lite.h"
...
DECLARE_FUNC(cryptonightzum) {
    ...
    if(fast)
        cryptonightzum_fast_hash(input, output, input_len);
    else {
        if (cn_variant > 0 && input_len < 43)
            RETURN_EXCEPT("Argument must be 43 bytes for monero variant 1+");
        cryptonightzum_hash(input, output, input_len, cn_variant);
    }
    SET_BUFFER_RETURN(output, 32);
}

DECLARE_FUNC(cryptonightzumlite) {
    DECLARE_SCOPE;
    ...
    if(fast)
        cryptonightzumlite_fast_hash(input, output, input_len);
    else {
        if (cn_variant > 0 && input_len < 43)
            RETURN_EXCEPT("Argument must be 43 bytes for monero variant 1+");
        cryptonightzumlite_hash(input, output, input_len, cn_variant);
    }
    SET_BUFFER_RETURN(output, 32);
}
...
DECLARE_INIT(init) {
    ...
    NODE_SET_METHOD(exports, "cryptonightzum", cryptonightzum);
    NODE_SET_METHOD(exports, "cryptonight-zum", cryptonightzum);
    NODE_SET_METHOD(exports, "cryptonightzumlite", cryptonightzumlite);
    NODE_SET_METHOD(exports, "cryptonight-zum-lite", cryptonightzumlite);
